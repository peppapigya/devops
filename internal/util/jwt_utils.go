...
func GenerateJwtToken(ID uint, username string, roles []string) (string, error) {
	...
	claims := Claims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Second * time.Duration(jwtProperties.ExpireTime))), // 过期时间
			IssuedAt:  jwt.NewNumericDate(time.Now()),                                                            // 签发时间
			NotBefore: jwt.NewNumericDate(time.Now()),                                                            // 生效时间
			Issuer:    "k8s-platform-go",                                                                         // 签发人，也就是区分服务用的
		},
	}
	// 生成token
...
func ParseToken(tokenStr string) (*Claims, error) {
	...
	token, err := jwt.ParseWithClaims(tokenStr, &claims, func(token *jwt.Token) (interface{}, error) {
		return []byte(config.GetGlobalConfig().Jwt.Secret), nil
	})
	if err != nil {
		return nil, err
	}
